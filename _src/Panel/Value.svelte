<script>
  ...
  import { round, formatNumber, substituteVars } from "./format";
  ...
  $: showPrevValue =
    (Number.isFinite(prevValue) &&
      resolvedValue !== null &&
      Math.abs(round(prevValue - resolvedValue, digits)) >= minDiff) ||
    forcePrev;
  $: prevFormatted =
    (configStore,
    $configStore,
    prevValue && Number.isFinite(prevValue)
      ? (prevLabel ? prevLabel + ": " : "") +
        formatNumber(prevValue, digits, prevWithSign) +
        suffix
      : "");
  $: prevLabelFormatted =
    (configStore,
    ...
    prevValue && Number.isFinite(prevValue)
      ? (prevLabel ? prevLabel + ": " : "") +
        formatNumber(prevValue, digits) +
        suffix
      : "");
  $: prevDiff = Number.isFinite(prevValue)
    ? (prevAbsolute ? prevValue : resolvedValue - prevValue) *
      (reversePrevSign ? -1 : 1)
    : null;
  $: prevDiffFormatted = Number.isFinite(prevDiff)
    ? (configStore,
      ...
      Number.isFinite(prevDiff)
        ? formatNumber(prevDiff, digits, !prevAbsolute) +
          (suffixPrev ? suffixPrev : suffix)
        : "")
    : null;
  $: prevClass =
    (prevDiff !== null
    ...
    " prev";
  $: mainClass =
    useColorsForValue && resolvedValue
      ? resolvedValue > minValue
        ...
        : "zero"
      : "value";
  $: prevTitleFormatted = substituteVars(prevTitle ? prevTitle : "${value}", {
    value: prevLabelFormatted,
  });
</script>
...
>{#if showPrevValue}
  <small class={`has-pointer-events ${prevClass}`} title={prevTitleFormatted}
    ><slot
      ...
      diff={prevDiff}
      diffFormatted={prevDiffFormatted}>{prevDiffFormatted}</slot
    ></small
  >{/if}
